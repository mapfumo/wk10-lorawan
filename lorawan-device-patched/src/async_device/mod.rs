    ...
    G: RngCore,
{
    ...
    class_c: bool,
    // Store last received SNR and RSSI for diagnostics
    last_snr: i8,
    last_rssi: i16,
}
    ...
    G: RngCore,
{
        ...
        while let Some(timeout_fut) = maybe_timeout_fut.take() {
            ...
            .await
            {
                RxcWindowResponse::Rx(sz, q, timeout_fut) => {
                    // Store SNR and RSSI for diagnostics
                    self.last_snr = q.snr();
                    self.last_rssi = q.rssi();
    ...
    /// Get the last received SNR (Signal-to-Noise Ratio) in dB
    pub fn last_snr(&self) -> i8 {
        self.last_snr
    }
    
    /// Get the last received RSSI (Received Signal Strength Indicator) in dBm
